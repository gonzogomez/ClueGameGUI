...
public class RaderInitTests {
	...
	@Test
	// Test a doorway in each direction, plus two cells that are not doorways.
	public void FourDoorDirections() {
		// Test one each RIGHT/LEFT/UP/DOWN
		RoomCell room = board.getRoomCellAt(4, 3);
		assertTrue(room.isDoorway()); //10/21 old: room.isDoorway(room
		...
		assertFalse(cell.isDoorway());	//altered line	

	}
	
	@Test
	//Tests number of Doorways and walkways in map.
	public void testNumberOfDoorways() 
	{
		...
		for (int i=0; i<totalCells; i++) {
			...
			//System.out.println(cell + ", i: " + i);
			if (cell.isDoorway())					// original: if (cell.isDoorway())
				numDoors++;
			if(cell.isWalkway())	{				//mikes test
				//System.out.println("found a walkway at index: " + i );
				numWalkways++;
			}
		}
		//System.out.println("Walkways: " + numWalkways);
		Assert.assertEquals(172,  numWalkways);
